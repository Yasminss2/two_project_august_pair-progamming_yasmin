...
COLOR_VER_ESC = "#480e0e"  # AE (Fundo das abas e headers)
COLOR_VER_MED = "#8b2121"  # AM (Bordas e detalhes)
COLOR_VER_CLA = "#d74c9b"  # AC (Destaque e fundos secundários)
COLOR_VERDE = "#a6c844"  # V  (Botão de Entrada / Sucesso)
COLOR_ROSA = "#b83764"  # R  (Botão de Saída / Alertas)
COLOR_ROSINHA = "#f083d6"  # A  (Destaque de Cripto / Seleção de Aba)
COLOR_ACO = "#16940d"  # B  (Texto escuro / Fundo principal)
COLOR_BRANCO = "#FEFDFF"
...
janela.configure(bg=COLOR_VER_ESC)

# Estilo para Abas (TTK)
style = ttk.Style()
style.theme_use("default")
style.configure("TNotebook", background=COLOR_VER_ESC)
style.configure(
    "TNotebook.Tab",
    background=COLOR_VER_MED,
    foreground="white",
    padding=[12, 6],
    font=("Arial", 10, "bold"),
)
style.map(
    "TNotebook.Tab",
    background=[("selected", COLOR_ROSINHA)],
    foreground=[("selected", COLOR_BRANCO)],
)

# Header Superior
header = tk.Frame(janela, bg=COLOR_VER_ESC, height=50)
header.pack(fill="x")
lbl_titulo = tk.Label(
    header,
...
ent_valor_conta = tk.Entry(
    ...
    relief="solid",
    bd=1,
    highlightbackground=COLOR_VER_MED,
)
...
lst_extrato = tk.Listbox(
    ...
    fg=COLOR_ACO,
    bg="#F9F9F9",
    selectbackground=COLOR_VER_CLA,
    relief="solid",
    bd=1,
)
